...
const fetchData = async value => {
  ...
  const response = await axios.get(
    'https://siemoo.vercel.app/api/v1/artikel/limbah',
    {headers},
  );
...
export default function Sulap() {
  Sulap.displayName = 'Sulap';
  ...
  return (
    <SafeAreaView
      ...
      <View className="w-[95%] mt-10">
        <TopTitleMenu title={'Sulap Limbah'} />
        ...
        <ScrollView className="h-[80%]">
          ...
          {data.map((data, index) => {
            return (
              <ArtikelDaftar data={data} index={index} />
            );
          })}
        </ScrollView>
        ) : (
